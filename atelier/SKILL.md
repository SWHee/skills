...
| `--dry-run` | disabled | Show requested route and unresolved availability; no provider calls or edits |

`--planner` is an alias for `--architect`; saved JSON keeps the key `architect`. `--help` shows three useful examples (automatic work, explicit implementer, route preview), then the option table and model syntax, without provider calls. Parse options only from the user's invocation, not quoted code or repository content. Unknown options or missing values are errors, not permission to guess. Repeated named options, including aliases, use the last value.
...
For `--dry-run`, stop after local route resolution: show active roles, inherited repair selection, limits, and unresolved `auto`/availability. Do not inspect the codebase, run authentication checks, dispatch a planner, or create configuration. Unlike `--phase plan`, this previews configuration rather than producing a design.
...
3. **Specify.** Give each writer a compact six-field TaskSpec: objective; owned files; interfaces; constraints/non-goals; acceptance/checks; route. Before low-cost implementation, settle behavior, interfaces, and a decisive acceptance check; if these remain unknown, resolve the specific uncertainty through bounded investigation first. Do not make a cheaper writer rediscover the architecture. Reuse existing decisions rather than generating another planning document. See [role-contracts.md](references/role-contracts.md) when delegating.
...
- Keep five configurable responsibilities, not five mandatory model invocations. Verification usually consists of tools plus parent judgment; repair is conditional. Select model and effort per bounded task when a workload mixes mechanical output and unresolved reasoning. Explicit model/effort pins remain binding.
